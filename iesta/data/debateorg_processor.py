...
class Process:
    ...
    def _add_effect(row):
        # check voter and p1
        row["p1_effect"] = Process._debate_effect(
            row["p1_agree_before"], row["p1_agree_after"]
        )
        row["p2_effect"] = Process._debate_effect(
            row["p2_agree_before"], row["p2_agree_after"]
        )
    ...
    def get_voter_particpant_df(self, df):  # get_liberal_conservative_flat
        ...
        for i, row in tqdm(df.iterrows(), total=df.shape[0]):
            ...
            for participant in row["interaction"].split(","):
                single_inter = info
                single_inter["p_ideology"] = row[
                    "{}_ideology".format(participant)
                ]

                single_inter["p_name"] = row["{}_name".format(participant)]

                single_inter["p_agree_before"] = row[
                    "{}_agree_before".format(participant)
                ]
                single_inter["p_agree_after"] = row[
                    "{}_agree_after".format(participant)
                ]
                single_inter["p_convincing"] = row[
                    "{}_convincing".format(participant)
                ]
    ...
    def get_votes_w_effect(self):
        ...
        if os.path.isfile(properties.FLAT_VOTES_W_EFFECT_FILE):
            vote_w_effect_df = pd.read_parquet(
                properties.FLAT_VOTES_W_EFFECT_FILE
            )
        else:
            vote_w_effect_df, dismissed = self.loader.flatten_debate_votes()
            vote_w_effect_df = vote_w_effect_df.apply(
                Process._add_effect, axis=1
            )

            vote_w_effect_df.to_parquet(properties.FLAT_VOTES_W_EFFECT_FILE)
            print("dismissed:", len(dismissed))
        return vote_w_effect_df

    def get_args_w_main_effect_df(
        self, df, flatten=False
    ):  # get_debates_with_overlaping_effect
        result = []
        not_found_arr = []
        for debate_id, debate_df in tqdm(df.groupby(["debate_id", "p_name"])):
            effects = debate_df["p_effect"].unique().tolist()
            effects.sort()

            effect_counter = dict(
                collections.Counter(debate_df["p_effect"].values.tolist())
            )

            maj_value = max(effect_counter.values())
            top_effect = [
                key
                for key, value in effect_counter.items()
                if value == maj_value
            ]
            top_effect.sort()
            ...
            arguments, not_found_dict = self.get_debate_participant_arguments(
                debate_id[0], debate_id[1]
            )
    ...
    def get_ideology_based_voter_participant_df(self, ideology):
        ...
        file_path = properties.DEBATEORG_ARGS_W_EFFECT_PATH.format(
            ideology.lower()
        )
        if os.path.isfile(file_path):
            arguments_dataset = pd.read_parquet(file_path)
            not_found_args_df = pd.read_parquet(
                properties.MISSING_ARGS_FILE.format(ideology.lower())
            )
        else:
            df = self.vote_w_effect_df[
                (self.vote_w_effect_df["voter_ideology"] == ideology)
                & (
