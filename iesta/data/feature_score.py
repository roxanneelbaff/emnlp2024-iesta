...
def generate_training_w_scores(ideology: str = "liberal",
                               save: bool = True):
    result = {}
    try:
        result["effective"] = pd.read_parquet(f"data/training/strategy_feature_based/{ideology}_effective.parquet")
        result["ineffective"]  = pd.read_parquet(f"data/training/strategy_feature_based/{ideology}_ineffective.parquet")
    except Exception:
