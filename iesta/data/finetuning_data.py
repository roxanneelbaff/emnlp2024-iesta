...
class FineTuningData:
    ...
    @staticmethod
    def get_template( instruction, new_system_prompt: str = IESTA_SYSTEM_PROMPT ):
        SYSTEM_PROMPT = FineTuningData.B_SYS + new_system_prompt + FineTuningData.E_SYS
        prompt_str = FineTuningData.B_INST + SYSTEM_PROMPT + instruction + FineTuningData.E_INST
        return prompt_str
    ...
    def undersample_df(df, ideology, min_count: int = 1000):
        ...
        df_effective = df_effective.apply(add_score, axis=1, args=(scores["effective"], ))
        df_effective = df_effective.sort_values(by=["features_score"], ascending=False)

        df_ineffective = df[df['label'] == 0]
        df_ineffective = df_ineffective.apply(add_score, axis=1, args=(scores["ineffective"], ))
        df_ineffective = df_ineffective.sort_values(by=["features_score"], ascending=True)

        min_count = min_count if min_count > 0 else min(len(df_ineffective),
                                                        len(df_effective))
    ...
    def undersample(df, min_count = -1):
        ...
        print(
            np.unique(
                np.array(df["label"]), return_counts=True
            )
        )
        ...
        print(
            np.unique(
                np.array(df_undersampled["label"]), return_counts=True
            )
        )
    ...
    @staticmethod
    def get_filtered_resampled_data(ideology, min_count = 1000,
        ...
        for split in ['training', 'validation', 'test']:
            ...
            ideology_dataframes[split] = FineTuningData.undersample(
                ds,
                min_count=min_count).to_pandas() if undersample else ds.to_pandas()
    ...
    def get_sv_data(ideology, skip_system_prompt=False, save: bool =True):
        ...
        def _add_prompt(row):
            training_prompt_template: str = FineTuningData.get_template(
                ...
                new_system_prompt="" if skip_system_prompt else FineTuningData.IESTA_SYSTEM_PROMPT
                )
            row["prompt"] = training_prompt_template.format(
                text=row["text"],
                effectiveness="ineffective" if int(row["label"]) == 0 else "effective")
            row["prompt_len"] = len(row["prompt"])
            return row
        
        print(f"1/2 Fetching, filtering and resampling data: {ideology.capitalize()}")
        training_data = FineTuningData.get_filtered_resampled_data(ideology,
                                                              undersample=False)["training"]
        ...
        if save:
            skip_system_prompt_str = "_with_system_prompt" if not skip_system_prompt else "_without_system_prompt"
            eff.to_parquet(f"data/training/{ideology}{skip_system_prompt_str}_prompt_effective.parquet")
            ineff.to_parquet(f"data/training/{ideology}{skip_system_prompt_str}_prompt_ineffective.parquet")
        return eff, ineff
    ...
    def get(prompt_format:str = "classification"):
        ...
        liberal_dfs_dict = FineTuningData.get_filtered_resampled_data("liberal")

        print("2/2 Fetching, filtering and resampling data: Conservative")
        conservative_dfs_dict = FineTuningData.get_filtered_resampled_data("conservative")
        ...
        for split in liberal_dfs_dict.keys():
            ...
            combined_df = pd.concat([liberal_df[["instruct_format", "output"]],
            ...
            combined_shuffled_df = combined_df.sample(frac=1,
                                                      replace=False,
                                                      ignore_index=True,
                                                      random_state=42
                                                      )
            assert len(combined_df) == len(combined_shuffled_df)
            combined_shuffled_df = combined_shuffled_df.drop_duplicates(keep='last')
            print(f"dropped {len(combined_df)-len(combined_shuffled_df)} duplicates for {split} data ")
