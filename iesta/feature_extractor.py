...
def extract_nrc_emotion(ideology):
    ...
    arguments_df, _ = processor.get_ideology_based_voter_participant_df(
        ideology
    )
...
def extract_mpqa_arg(ideology):
    ...
    mpqa_arg_df = lexicons.count_mpqa_arg(
        mpqa_arg_df, text_column="argument", prefix="mpqa_arg_"
    )
...
def extract_empath(ideology):
    ...
    empath_df = lexicons.count_empath(
        empath_df, text_column="argument", prefix="empath_"
    )
...
def extract_empath_ideology(ideology):
    ...
    print("calculating lexicon cound for empath...")
    empath_df = lexicons.count_empath_for_ideologies(
        empath_df, text_column="argument", prefix="empath_ideology_"
    )
    empath_df.to_parquet(file_path, index=False)
    return empath_df
