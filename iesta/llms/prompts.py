...
_IDEOLOGY_WRITER_PROMPT_ = " written by a writer with a non-{ideology} ideology,"
...
def get_instructions_per_combo(combo: list, ideology: str) -> str:
    ideology_writer: str = _IDEOLOGY_WRITER_PROMPT_.format(ideology=ideology) if "ideology" in combo else ""
    ideology_reader: str = _IDEOLOGY_READER_PROMPT_.format(ideology=ideology) if "ideology" in combo else ""
    content: str = _CONTENT_PROMPT_ if "content" in combo else ""
    style: str = _STYLE_PROMPT_ if "style" in combo else ""

    instructions = _INSTRUCTIONS_.format(ideology_writer=ideology_writer,
...
def get_all_instructions_per_ideology(ideology) -> dict:
    ...
    all_instructions["base"] = get_instructions_per_combo([], ideology)+"Ineffective argument: ```{text}```"
