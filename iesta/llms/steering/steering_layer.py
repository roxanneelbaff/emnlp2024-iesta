...
class SteeringLayer(torch.nn.Module):
    def __init__(self, layer_of_interest):
        ...
        if next(self.layer_of_interest.parameters()).device.type == "cuda":
            self.device = torch.device(
                f"{next(self.layer_of_interest.parameters()).device.type}:{next(self.layer_of_interest.parameters()).device.index}"
            )
        else:
            self.device = torch.device("cpu")

        self.steering_vector = torch.nn.Parameter(
            torch.nn.init.xavier_normal_(
                torch.empty((1, 4096), device=self.device, dtype=torch.float32)
            )
        )
    ...
    def forward(self, *x):
        if self.shift_with_new_idea:
            return self.layer_of_interest(*x) + self.b * (
                self.steering_vector - self.layer_of_interest(*x)
            )

        if self.add_steering:
            if self.ignore_activations:
                return (
                    self.layer_of_interest(*x) * 0.0
                    + self.b * self.steering_vector
                )
            return (
                self.a * self.layer_of_interest(*x)
                + self.b * self.steering_vector
            )
