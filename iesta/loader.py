...
class Loader():
    ...
    def flatten_debate_votes(self):
        ...
        result_arr = []
        debates_err_arr = []
        for i, row in tqdm(self.debates_df.iterrows(), total = self.debates_df.shape[0]):
...
def get_args_w_effect(ideology):
    ...
    args_df = pd.read_parquet(properties.DEBATEORG_ARGS_W_EFFECT_PATH.format(ideology))
    return args_df
