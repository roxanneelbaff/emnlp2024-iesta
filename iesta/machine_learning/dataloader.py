...
def get_previous_opposite_arg(df, debate_id, p_name, debate_round, text_col):
    ...
    if debate_round > 0:
        prev_arg_df = df[
            ...
            & ~(df["p_name"] == p_name)
            & (df["round"] == (debate_round - 1))
        ]
        if len(prev_arg_df) == 0:
            with open("../data/error_no_previous_args.txt", "a") as f:
                f.write(
                    f"{debate_id},  p_name:{p_name}, round:{debate_round}\n"
                )

        prev_arg = (
            prev_arg_df[text_col].values[0] if len(prev_arg_df) > 0 else ""
        )
    return prev_arg
...
def apply_add_prev_arg(row, original_df):
    ...
    row[
        "current_arg_w_previous"
    ] = f"{previous_opposite_arg} <MAIN-ARG> {row['argument']}"
...
def _get_rand_sample_indices(populaion, sample_num, random_state=42):
    return sample_without_replacement(
        populaion, sample_num, random_state=random_state
    )
...
class IESTAData:
    ...
    keep_labels: list = dataclasses.field(
        default_factory=lambda: LABELS.EFF_INEFF
    )
    ...
    def _get_out_files_path(self):
        return os.path.join(
            properties.ROOT_PATH, "splitted", f"methodology_{self.methodology}"
        )

    def split_iesta_dataset_by_debate(
        self, force_reload: bool = False, profile: bool = False
    ):
        """
        parammeters:
        ...
        def _apply_add_evaluation_classifier_data(row, debates):
            row["is_for_eval_classifier"] = (
                True if row["debate_id"] in debates else False
            )
            return row

        def _apply_no_punc(row, col):
            row["text_no_punc"] = re.sub(r"[^\w\s]", "", row[col])
            row["text_no_punc_on_clean"] = re.sub(
                r"[^\w\s]", "", row["cleaned_text"]
            )
            return row
        ...
        if os.path.isfile(file_path) and not force_reload:
            ...
            df, _ = processor.get_ideology_based_voter_participant_df(
                self.ideology
            )
            ...
            with open(
                os.path.join(properties.ROOT_PATH, "dismiss_text.txt"), "r"
            ) as dismissedf:
                dissmiss_arr = list(
            ...
            df = df[
                ~df["text_no_punc_on_clean"].str.lower().isin(dissmiss_arr)
            ]
            ...
            if profile:
                ...
                profile.to_file(
                    os.path.join(
                        properties.ROOT_PATH,
                        "profilers",
                        f"{self.ideology}_all_data_low.html",
                    )
                )

                profile = ProfileReport(
                    df[["cleaned_text"]], title="Profiling Report"
                )
                profile.to_file(
                    os.path.join(
                        properties.ROOT_PATH,
                        "profilers",
                        f"{self.ideology}_all_data.html",
                    )
                )
            ...
            training_debate, testval_debates = train_test_split(
                debates,
                test_size=self.test_split,
                random_state=self.random_state,
                shuffle=True,
            )
            validation_debates, test_debates = train_test_split(
                testval_debates,
                test_size=(1 / 3),
                random_state=self.random_state,
                shuffle=True,
            ...
            splits_df_lst.append(
                pd.DataFrame(
                    {
                        "debate_id": training_debate,
                        "split": ["training"] * len(training_debate),
                    }
                )
            )
            splits_df_lst.append(
                pd.DataFrame(
                    {
                        "debate_id": validation_debates,
                        "split": ["validation"] * len(validation_debates),
                    }
                )
            )
            splits_df_lst.append(
                pd.DataFrame(
                    {
                        "debate_id": test_debates,
                        "split": ["test"] * len(test_debates),
                    }
                )
            )
            ...
            data_w_splits_df = df.merge(
                right=debate_split_df,
                how="left",
                on="debate_id",
                suffixes=("", "extra"),
            )
            ...
            for split in ["training", "validation", "test"]:
                assert len(
                    data_w_splits_df[data_w_splits_df["split"] == split][
                        "debate_id"
                    ].unique()
        ...
        if "is_for_eval_classifier" not in data_w_splits_df.columns.tolist():
            ...
            training_debates_sample = _get_sample_debates(
                data_w_splits_df, "training"
            )
            validation_debates_sample = _get_sample_debates(
                data_w_splits_df, "validation"
            )
            test_debates_sample = _get_sample_debates(data_w_splits_df, "test")
            sample_debates = (
                training_debates_sample
                + validation_debates_sample
                + test_debates_sample
            )
            data_w_splits_df = data_w_splits_df.apply(
                _apply_add_evaluation_classifier_data,
                axis=1,
                args=(sample_debates,),
            )
        data_w_splits_df = data_w_splits_df[
            [
                "id",
                "debate_id",
                "p_name",
        ...
        split_effect_pivot_df = pd.crosstab(
            data_w_splits_df["split"], data_w_splits_df["effect"]
        )
    ...
    def prepare_data_for_transformers(self, reload: bool = False):
        (
            data_w_splits_df,
            split_effect_pivot_df,
        ) = self.split_iesta_dataset_by_debate()
        ...
        df_file = os.path.join(
            path, f"processed_data_{self.ideology.lower()}.parquet"
        )
        ...
        for effect_split, effect_split_df in data_w_splits_df.groupby(
            ["effect", "split"]
        ):
            # for an effect
            args_lst = []
            ...
            file = os.path.join(
                path,
                f"{self.ideology.lower()}_{effect_split[0]}_{effect_split[1]}.txt",
            )
            ...
            if self.methodology == METHODOLOGY.EACH:
                ...
                for group, df_ in effect_split_df.groupby(
                    ["debate_id", "p_name"]
                ):
                    debate_round = (
                        df_["round"].min()
                        if self.methodology == "first"
                        else df_["round"].max()
                    )
                ...
                for _, df_ in effect_split_df.groupby(["debate_id", "p_name"]):
                    df_ = df_[
                        (df_["round"] == df_["round"].min())
                        | (df_["round"] == df_["round"].max())
                    ]
                    df_ = df_.sort_values(by="round")

                    arg = " <LAST> ".join(df_[text_col_name].tolist())
                    assert (
                        len(df_[text_col_name].tolist()) == 2
                        if (df_["round"].min() != df_["round"].max())
                        else len(df_[text_col_name].tolist()) == 1
                    )
    ...
    def get_training_data(self, reload=False):
        ...
        training_data = training_data[
            (
                (training_data["is_for_eval_classifier"] == False)
...
def load_training_data(
    ideology: str = "liberal",
    keep_labels=LABELS.EFF_INEFF,
    methodology: str = METHODOLOGY.EACH,
) -> Dict[str, pd.DataFrame]:
    dataloader = IESTAData(
        ideology=ideology, keep_labels=keep_labels, methodology=methodology
    )
...
def load_training_features_df(
    ...
    transformers_based_features_df = get_features_df(
        transformer_features_path, 100, training_data
    )
