...
class IESTAFeatureExtractionPipeline(Pipeline):
    ...
    def process_input(self) -> list:
        ...
        txt_df =txt_df.rename(columns={self.argument_col: "text"},)
    ...
    def init_and_run(self):
        if self.exec_transformer_based:
            ...
            self.add_annotation_pipe(name = "MpqaPipeOrchestrator",    save_output= True, is_spacy=True)
...
def extract_features(ideology:str, 
                        data_path:str, 
                        batch_size:int=500, 
    ...
    pipeline = IESTAFeatureExtractionPipeline(save_output= True, exec_transformer_based=transformer_based_features)
    pipeline.spacy_n_processors= 1 if transformer_based_features else spacy_n_processors
...
def get_features_df(files, batch_size, training_data) -> pd.DataFrame:
    ...
    #assert len(features_df) == len(original_df)
    df_= pd.merge(training_data,features_df,left_index=True,right_index=True)
