...
def run(ideology, models, examples_k, limit):
    ...
    for k in range(0, examples_k + 1):
        for model in models:
            iestal_model = (
                LlamaV2(model_name_path="meta-llama/Llama-2-7b-chat-hf")
                if model == "llamav2"
                else ChatGpt()
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--ideology", type=str, default="all"
    )  # liberal or conservative
    parser.add_argument(
        "-k", "--examples_k", type=int, default=0
    )  #  shots, 1 means run 0 and 1 shot
    parser.add_argument(
        "-m", "--models", type=str, default="all"
    )  ## llamav2, chatgpt, + support models with activations
    parser.add_argument(
        "-l", "--limit", type=int, default=-1
    )  # number of text samples for ineffective arguments
