...
class TextClassificationWAccelerate:
    ...
    mixed_precision: str = (
        None  # "fp16"  #["no", "fp16", "bf16", "fp8"] or None
    )
    ...
    def load_training_checkpoint(self, load_dir, tag=None, **kwargs):
        ...
        The main purpose for this is to be able to resume training from that instant again
        """
        _, checkpoint_state_dict = self.model.load_checkpoint(
            load_dir, tag=tag, **kwargs
        )
    ...
    def __post_init__(self):
        self.experiment = TextClassificationWAccelerate.get_experiment(
            self.run_id
        )
    ...
    def resample_ds(self, split):
        ...
        print(
            np.unique(
                np.array(self.dataset[split]["label"]), return_counts=True
            )
        )
        ...
        print(
            "undersampling so all each class in the training set has length"
    ...
    def set_batch_size(self):
        ...
        if (
            ...
            and self.accelerator.distributed_type != DistributedType.TPU
        ):
            self.gradient_accumulation_steps = (
                self.batch_size
                // TextClassificationWAccelerate.MAX_GPU_BATCH_SIZE
            )
            self.batch_size = TextClassificationWAccelerate.MAX_GPU_BATCH_SIZE
    ...
    def _tokenize_function(self, examples):
        ...
        if self.uncase:
            examples[self.text_col] = [
                text.lower() for text in examples[self.text_col]
            ]
    ...
    def evaluate(self, split_dataloader, extra_params):
        ...
        eval_metric = self.evaluator.compute(
            predictions=None, references=None, **extra_params
        )
        return eval_metric
    ...
    def train(self):
        try:
            ...
            extra_params = (
                {"average": self.averaged_metric}
                if self.averaged_metric is not None
                else {}
            )
            ...
            self.accelerator.print(
                "*************** after setting model to accelerate **************"
            )
            helpers.print_gpu_utilization()
            ...
            for epoch in range(num_epochs):
                ...
                eval_metric = self.evaluate(
                    self.eval_dataloader, extra_params
                )[self.metric]

                self.accelerator.print(
                    f"epoch {epoch} - {self.metric}", eval_metric
                )

                try:
                    if self.experiment is not None:
                        self.experiment.log_metric(
                            self.metric, eval_metric, epoch=epoch
                        )
                ...
                if self.output_dir is not None:
                    epoch_checkpoint_dir = os.path.join(
                        self.output_dir, epoch_checkpoint_dir
                    )
                ...
                if best_metric is None or best_metric < eval_metric:
                    ...
                    self.accelerator.print(
                        f"New best metric: {best_metric} at epoch {epoch}"
                    )
                    self.accelerator.print(
                        f"best_metric_checkpoint: {best_metric_checkpoint}"
                    )
                    self.accelerator.print(
                        "removing previously saved checkpoints..."
                    )
                    ...
                    for f in folders:
                        if f != best_metric_checkpoint:
                            try:
                                ...
                                print(
                                    "error while trying to delete prev folders:",
                                    e,
                                )
            ...
            self.accelerator.print(
                "Loading best model at " f"{best_metric_checkpoint}"
            )
            self.accelerator.load_state(best_metric_checkpoint)
            re_eval = self.evaluate(self.eval_dataloader, extra_params)[
                self.metric
            ]
            self.accelerator.print(f"evaluation on eval: {re_eval}")
            test_eval_val = self.evaluate(self.test_dataloader, extra_params)[
                self.metric
            ]
    ...
    def predict_all(
        ...
        if use_current_model:
            ...
            classifier = pipeline(
                task, model=f"{self.model_org}/" f"{self.output_dir}"
            )
    ...
    def predict_1label(self, text: str, use_current_model: bool = True):
        ...
        model = AutoModelForSequenceClassification.from_pretrained(
            f"{self.model_org}/{self.output_dir}"
        )
        with torch.no_grad():
