...
conservative_hf = IESTAHuggingFace(conservative_data_obj, reload_preprocess=True)
conservative_w_effect = conservative_hf.upload_w_labels(is_for_style_classifier=False, force_reload=True)
conservative_w_effect_for_sc = conservative_hf.upload_w_labels(is_for_style_classifier=True, force_reload=True)

liberal_data_obj = IESTAData(ideology="liberal", keep_labels = LABELS.EFF_INEFF)
liberal_hf = IESTAHuggingFace(liberal_data_obj, reload_preprocess=True)
