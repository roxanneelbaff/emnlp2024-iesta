...
def load_nrc_emotions(filepath=NRC_FILEPATH):
    emolex_df = pd.read_csv(
        filepath, names=["word", "emotion", "association"], sep="\t"
    )
...
def count_nrc_emotions_and_sentiments(
    df, text_column="text", path=NRC_FILEPATH, prefix="nrc_"
):
    ...
    result_df = result_df.apply(
        ...
        axis=1,
        args=(emotions, sentiments, nrc_words_df, text_column, path, prefix),
    )
    return result_df
...
def _count_empath_based_on_ideology(row, text_column, empath_lexicon, prefix):
    empath_dic = empath_lexicon.analyze(
        str(row[text_column]),
...
def count_empath_for_ideologies(
    df, text_column="text", prefix="empath_ideology_"
):
    lexicon = Empath()
    ...
    result = df.apply(
        _count_empath_based_on_ideology,
        axis=1,
        args=(text_column, lexicon, prefix),
    )
    return result
